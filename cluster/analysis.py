...
def _reduce_tree(tree, reduce_func, value_func=lambda node: node.id):
    """
    ...
    Equivalent to:
    ```
    def reduce(tree, func, value_func):
    if tree.is_leaf():
        return value_func(tree)
    else:
        left = reduce(tree.get_left(), func)
        right = reduce(tree.get_right(), func)
        return func(left, right)
    ```
    :param tree: root node of the tree
    :type tree: `scipy.cluster.hierarchy.ClusterNode`
    :param reduce_func: function that will be performed on reduction
    :param value_func: function that gets the value of a `ClusterNode` defaults to retrieving the node's id
...
class Cluster(object):
    ...
    def index(self):
        if self._item_indices is None:
            self._item_indices = pd.Index(self._item_ids)
    ...
    def average(self):
        def reduce_function(x, y):
            path_average = dtw.transformations.dtw_path_averaging(x, y)
            max_len = max(len(x[~np.isnan(x)]), len(y[~np.isnan(y)]))
            return dtw.transformations.shrink_to_length(path_average, max_len)

        items_ix = self.items.ix
        value_function = lambda x: items_ix[x.id].values
        return _reduce_tree(self.root, reduce_function, value_function)
