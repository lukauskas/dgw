...
def argument_parser():
    ...
    group = parser.add_mutually_exclusive_group('Mode', required=True)

    group.add_argument('-g', '--gene', help='Output whole gene', action='store_const', const=True, default=False)
    group.add_argument('-e', '--exon', metavar='EXON_NUMBER', help='Output the exon EXON_NUMBER (0-based - use "0" for first exon) of each gene',
                        default=None, type=int)
    group.add_argument('-s', '--splicing-site', metavar='EXON_NUMBER', help='Output the splicing site of the exon EXON_NUMBER (0-based again)',
                       default=None, type=int)                        )
...
def main():
    ...
    elif args.splicing_site:
        # We are using hacky way of getting these splicing site locations
        splicing_sites = genes.regions_around_first_splicing_site(args.splicing_site, 0)
